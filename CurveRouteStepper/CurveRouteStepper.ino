// ============================================================
// Scheme 2: Arc driving with differential steering
// Runs on Arduino UNO R4 WiFi
// Uses FspTimer hardware ISR + Direct ICM-20948 + ArduinoBLE
// Positive angle = turn RIGHT
// All parameters tunable via BLE (SciolyEV_Dashboard_v2.html)
// Commands: KP | BIAS | DIST | RADIUS | ANGLE | TIME | RESET
// BLE commands use event callback (reliable on R4)
// ============================================================
...
const float TRACK_WIDTH_MM    = 150.0; // Measure left to right wheel center distance!
const float ACCEL_TIME_SEC    = 2.0;
const float MM_PER_REV        = PI * WHEEL_DIAMETER_MM;

// ============================================================
// Runtime-tunable parameters (all updated via BLE)
// ============================================================
volatile float         rt_KP_HEADING         = 0.0;
volatile float         rt_BIAS               = 0.0;
volatile float         rt_DIST_CORRECTION_MM = 0.0;
volatile float         rt_ARC_RADIUS_MM      = 8056.25;
volatile float         rt_ARC_ANGLE_DEG      = 51.5;
volatile unsigned long rt_TARGET_TIME_MS     = 10000;
...
void recalcArc() {
  ...
  ACCEL            = LEFT_BASE_SPEED   / ACCEL_TIME_SEC;

  Serial.println("--- Arc recalculated ---");
  Serial.print("Radius:      "); Serial.print(rt_ARC_RADIUS_MM);    Serial.println(" mm");
  Serial.print("Angle:       "); Serial.print(rt_ARC_ANGLE_DEG);    Serial.println(" deg");
  Serial.print("Time:        "); Serial.print(rt_TARGET_TIME_MS);   Serial.println(" ms");
  Serial.print("Left arc:    "); Serial.print(LEFT_ARC_MM, 1);      Serial.println(" mm (outer)");
  Serial.print("Right arc:   "); Serial.print(RIGHT_ARC_MM, 1);     Serial.println(" mm (inner)");
  Serial.print("Left steps:  "); Serial.println(LEFT_TOTAL_STEPS);
  Serial.print("Right steps: "); Serial.println(RIGHT_TOTAL_STEPS);
  Serial.print("Left speed:  "); Serial.print(LEFT_BASE_SPEED, 1);  Serial.println(" st/s");
  Serial.print("Right speed: "); Serial.print(RIGHT_BASE_SPEED, 1); Serial.println(" st/s");
  Serial.println("------------------------");
}
...
void stepISR(timer_callback_args_t *args) {
  float accelStep = ACCEL / TIMER_FREQ;
  ...
  if (leftRunning && leftPosition < leftTotalSteps && leftCurrentSpeed > 0) {
    ...
    if (leftCounter >= 1.0) {
      ...
      if (leftPosition >= leftTotalSteps) {
        leftRunning = false; leftCurrentSpeed = 0;
      }
    }
  }
  ...
  if (rightRunning && rightPosition < rightTotalSteps && rightCurrentSpeed > 0) {
    ...
    if (rightCounter >= 1.0) {
      ...
      if (rightPosition >= rightTotalSteps) {
        rightRunning = false; rightCurrentSpeed = 0;
      }
    }
  }
}
...
bool vehicleStopped     = false; // True after run, cleared by RESET command

// ============================================================
// BLE Setup
// statusChar: Arduino → PC (notify)
...
// Registered with cmdChar.setEventHandler(BLEWritten, ...)
// Called automatically by ArduinoBLE when PC writes a command
// Much more reliable than cmdChar.written() polling on R4
// ============================================================
void onCmdReceived(BLEDevice central, BLECharacteristic characteristic) {
  ...
  if (key == "KP") {
    rt_KP_HEADING = val;
    Serial.print("BLE → KP: "); Serial.println(val);

  } else if (key == "BIAS") {
    rt_BIAS = val;
    Serial.print("BLE → BIAS: "); Serial.println(val);

  } else if (key == "DIST") {
    rt_DIST_CORRECTION_MM = val;
    Serial.print("BLE → DIST: "); Serial.println(val);
    recalcArc();

  } else if (key == "RADIUS") {
    rt_ARC_RADIUS_MM = val;
    Serial.print("BLE → RADIUS: "); Serial.println(val);
    recalcArc();

  } else if (key == "ANGLE") {
    rt_ARC_ANGLE_DEG = val;
    Serial.print("BLE → ANGLE: "); Serial.println(val);
    recalcArc();

  } else if (key == "TIME") {
    rt_TARGET_TIME_MS = (unsigned long)val;
    Serial.print("BLE → TIME: "); Serial.print(val); Serial.println(" ms");
    recalcArc();

  } else if (key == "RESET") {
    // Stop motors, clear run state, wait for button
    stopMotors();
    ...
    Serial.print("BLE → Unknown: "); Serial.println(cmd);
  }
}
...
bool icmInit() {
  for (int attempt = 1; attempt <= 3; attempt++) {
    ...
    if (whoAmI == 0xEA) {
      ...
      icmWriteRegister(2, ICM_GYRO_CFG,   0x31); delay(50); // DLPF 5.7Hz
...
void icmCalibrateGyro() {
  ...
  for (int i = 0; i < 200; i++) { icmReadGyroZ(); delay(5); } // Warmup
  long sum = 0;
...
// ============================================================
// Stop function
// Uses vehicleStopped flag instead of while(true)
// so BLE callback can still receive RESET after stopping
// ============================================================
void stopVehicle(const char* reason) {
  ...
  Serial.print("Left steps:      "); Serial.println(leftPosition);
  Serial.print("Right steps:     "); Serial.println(rightPosition);
  Serial.print("Arc traveled:    "); Serial.print(angleTraveled, 1); Serial.println(" deg");
  Serial.print("Final gyro:      "); Serial.print(headingAngle, 2); Serial.println(" deg");
  ...
  // Flag stops loop() from running motor logic
  // BLE callback (onCmdReceived) remains active to handle RESET
  vehicleStopped = true;
  vehicleStarted = false;
}
...
void setup() {
  ...
  pinMode(LEFT_DIR_PIN,   OUTPUT); digitalWrite(LEFT_DIR_PIN,   HIGH); // Left forward
  pinMode(RIGHT_DIR_PIN,  OUTPUT); digitalWrite(RIGHT_DIR_PIN,  LOW);  // Right forward (inverted)
  pinMode(LEFT_EN_PIN,    OUTPUT); digitalWrite(LEFT_EN_PIN,    HIGH); // Disabled until start
  pinMode(RIGHT_EN_PIN,   OUTPUT); digitalWrite(RIGHT_EN_PIN,   HIGH);
  ...
  if (!icmInit()) { Serial.println("ICM-20948 FAILED!"); while (true); }
  ...
  if (!BLE.begin()) { Serial.println("BLE FAILED!"); while (true); }
  ...
  // This is reliable on R4 unlike cmdChar.written() polling
...
void loop() {
  ...
  // --- If stopped: just keep BLE alive ---
  // onCmdReceived() handles RESET automatically via callback
  if (vehicleStopped) {
    BLE.poll();
    return;
  }

  // --- 1. Gyro read FIRST to keep dt accurate ---
  unsigned long now = millis();
  float dt = (now - lastTime) / 1000.0;
  lastTime = now;
  if (vehicleStarted) headingAngle += icmGetGyroZ_dps() * dt;

  // --- 2. Keep BLE alive (callback fires inside poll) ---
  BLE.poll();
  ...
  if (!vehicleStarted) {
    ...
    if (digitalRead(BUTTON_PIN) == HIGH) {
      ...
      if (digitalRead(BUTTON_PIN) == HIGH) {
        ...
        Serial.print("KP: ");     Serial.println(rt_KP_HEADING);
        Serial.print("BIAS: ");   Serial.println(rt_BIAS);
        Serial.print("RADIUS: "); Serial.println(rt_ARC_RADIUS_MM);
        Serial.print("ANGLE: ");  Serial.println(rt_ARC_ANGLE_DEG);
        Serial.print("TIME: ");   Serial.println(rt_TARGET_TIME_MS);
  ...
  // --- 4. Arc IMU correction ---
  // Expected heading based on step progress
  // Right turn = negative gyro convention
  float arcProgress     = leftPosition / (float)LEFT_TOTAL_STEPS;
  float expectedHeading = -(arcProgress * rt_ARC_ANGLE_DEG);

  // Positive error = turned too little (under-turning)
  // Negative error = turned too much  (over-turning)
  ...
  // Clamp to ±20% of each motor's base speed
  ...
  // --- 5. Serial debug every 200ms ---
  static unsigned long lastPrint = 0;
  if (millis() - lastPrint >= 200) {
    lastPrint = millis();
    Serial.print("Actual:"); Serial.print(headingAngle, 1);
    Serial.print(" Exp:");   Serial.print(expectedHeading, 1);
    Serial.print(" Err:");   Serial.print(headingError, 1);
    Serial.print(" Corr:");  Serial.print(kpCorrection, 1);
    Serial.print(" Arc:");   Serial.print(arcProgress * 100.0, 0); Serial.println("%");
  }
